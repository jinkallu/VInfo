	...
	public function execute() {
		...
		$html .= Html::rawElement( 'div', [ 'id' => 'mw-wrapper','style' => 'visibility: hidden;' ],
			Html::rawElement( 'div', [ 'class' => 'mw-body', 'id' => 'content', 'role' => 'main' ],
				...
				Html::rawElement( 'div', [ 'class' => 'mw-body-content' , 'style' => 'visibility: hidden;'],
					...
					/*.Html::rawElement( 'div', [ 'class' => 'parent' ],
						str_replace('Category', '', $this->get('catlinks'))
					) */
		...
		/*$res = $dbr->select(
			...
			array( 'cat_title', 'cat_pages' , 'cat_subcats'),            // $vars (columns of the table to SELECT)
			array('cat_title' => 'BigBang' ),                              // $conds (The WHERE conditions)
		...
		// parent
		$parent = 'BigBang';
		$html .= Html::openElement( 'div', [ 'class' => 'parent_hold' ] );
		$html .=  Html::rawElement(
			'div',
			[ 'class' => 'parent_root' ],
			$parent
		...
		// siblings
		$res = $dbr->select(
			'categorylinks',                                   // $table The table to query FROM (or array of tables)
			array( 'cl_from', 'cl_to' , 'cl_sortkey'),            // $vars (columns of the table to SELECT)
			array('cl_to' => $parent ),                              // $conds (The WHERE conditions)
			__METHOD__,                                   // $fname The current __METHOD__ (for performance tracking)
			array( 'ORDER BY' => 'cl_sortkey ASC' )        // $options = array()
		);
		$first_child_flag = false;
		
		$html .= Html::openElement( 'div', [ 'class' => 'siblings_holder' ] );
		foreach ( $res as $row ) {
			$res1 = $dbr->select(
				'page',                                   // $table The table to query FROM (or array of tables)
				array( 'page_title'),            // $vars (columns of the table to SELECT)
				array('page_id' => $row->cl_from ),                              // $conds (The WHERE conditions)
				__METHOD__,                                   // $fname The current __METHOD__ (for performance tracking)
			...
			foreach ( $res1 as $row1 ) {
				...
				$html .=  Html::rawElement(
					'div',
					[ 'class' => 'siblings' ],
					str_replace('_', ' ', $row1->page_title)
				) ;
			}
		}
		$html .= Html::closeElement( 'div' );

		$html .= Html::openElement( 'div', [ 'class' => 'children_holder' ] );
		// children
		$res = $dbr->select(
			'categorylinks',                                   // $table The table to query FROM (or array of tables)
			array( 'cl_from', 'cl_to' , 'cl_sortkey'),            // $vars (columns of the table to SELECT)
			array('cl_to' => $first_child->page_title),                              // $conds (The WHERE conditions)
			__METHOD__,                                   // $fname The current __METHOD__ (for performance tracking)
			array( 'ORDER BY' => 'cl_sortkey ASC' )        // $options = array()
		);
